...
The egress gateway feature allows for IPv4 connections originating from pods and
destined to specific cluster-external CIDRs to be routed through particular
nodes, from now on called "gateway nodes".
...
Specifically, when an egress gateway policy and a L7 policy both select the same
endpoint, traffic from said endpoint will not go through egress gateway, even if
...
When egress gateway is used in combination with XDP-based LB acceleration
(``--bpf-lb-acceleration=native``), the user must ensure that the host Iptables
configuration allows packets through the ``FORWARD`` chain. Full support will be
added in an upcoming release once :gh-issue:`19717` is resolved.
...
To work around this issue, Cilium can be instructed to install the necessary
IP rules and routes in order to route traffic through the appropriate
network-facing interface using the following subset of Helm values:

.. code-block:: yaml

  egressGateway:
    installRoutes: true  # Defaults to 'false'

or alternatively by setting ``install-egress-gateway-routes: true`` in the
...
The gateway node that should act as gateway for a given policy can be configured
...
Cilium is running on the device that has the the egress IP assigned to it, by
...
.. code-block:: yaml
  ...
  spec:
    ...
    egressGateway:
      ...
      # Otherwise it's possible to specify the interface to be used for egress traffic.
...
If you don't have an external service to experiment with, a very simple option
to demonstrate the functionality can be Nginx as the server access logs will
show from which IP address the request is coming.
...
.. parsed-literal::
    ...
    $ kubectl apply -f \ |SCM_WEB|\/examples/kubernetes-egress-gateway/egress-sample.yaml
...
IP (``192.168.60.100``) rather than one of the nodes in Kubernetes cluster:
